...
class Camap(object):
    ...
    def register(self, configjson):
        ...
        elif self.mode == "uparea":
            self.res = float(varDict["res"])
        else:
            raise KeyError(
                "undefined mode in configjson file: {0}".format(self.mode)
                )

    def mapgrid_hires(self, lons, lats, *args,
    ...
    def mapgrid_uparea(self, lons, lats, refupareas, buffer=5):
        upareamap = self.read_upareamap()
        grids = batch_upareamapper(upareamap, refupareas, lons, lats,
                                   self.nlon, self.nlat,
                                   self.west, self.south, self.res, buffer)
        outlons = [grid[0] for grid in grids]
        outlats = [grid[1] for grid in grids]
        errors = [grid[2] for grid in grids]
        return outlons, outlats, errors
    ...
    def read_upareamap(self, upareafile="uparea.bin", dtype=np.float32):
        path = os.path.join(self.mapdir, upareafile)
        size = os.path.getsize(path)
        size_assumed = self.nlat*self.nlon*4
        if size != size_assumed:
            raise RuntimeError(
                """file size is {0} bytes,
                but the instance assumes {1} bytes,
                from nlat {2} and nlon {3}""".format(size, size_assumed,
                                                     self.nlat, self.nlon)
                )
        upareamap = np.memmap(path, dtype=dtype, mode="r",
...
def upareamapper(upareamap, refuparea, lon, lat, nlon, nlat,
                 west, south, res, buffer, order="NS"):
    clon_init = int((lon - west)/res)
    if order == "NS":
        clat_init = nlat - int((lat - south)/res)
    elif order == "SN":
        clat_init = int((lat - south)/res)
    else:
        KeyError("{0}".format(order))
    error = 1e+20
    clonOut = np.nan
    clatOut = np.nan
    for ilat in range(max(0, clat_init-buffer), min(clat_init+buffer, nlat)):
        for ilon in range(max(0, clon_init-buffer), min(clon_init+buffer, nlon)):
            cerror = uparea_error(ilon, ilat, 0, 0, [refuparea], [upareamap])
            if cerror < error:
                clonOut = ilon
                clatOut = ilat
                error = cerror
    return clonOut, clatOut, error


def batch_upareamapper(upareamap, refupareas, lons, lats, nlon, nlat,
                       west, south, res, buffer):
    outputs = [upareamapper(upareamap, refupareas[i], lons[i], lats[i],
                            nlon, nlat, west, south, res, buffer)
               for i in range(len(lons))]
    return outputs
...
def uparea_error(clon, clat, idx, domidx,
